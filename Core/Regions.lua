...
function Regions.Apply(context, settings)
    ...
    for _, section in ipairs(context.song) do
        ...
        local startTime = Reaper.QNToTime(startQN)

        local endTime = Reaper.QNToTime(endQN)

        local color = nil

        if section.color then

            color = ColorsPalette[section.color]

            if not color then

                error("Unknown region color: " .. tostring(section.color))

            end

        end

        Reaper.AddRegion(section.name, startTime, endTime, color)
